...
func _process(delta: float) -> void:
	if active:
		...
		if Input.is_action_just_pressed("exitdiorama"):
			...
			tweenb.tween_property(Cam.maincam, "fov",90, 1.0)
			print(Cam.maincam.attributes.dof_blur_amount)
			await get_tree().create_timer(1.7).timeout
			Cam.fpsmode=true
			
			var tweenx = get_tree().create_tween()
			tweenx.tween_property(Cam.maincam, "attributes.dof_blur_amount",0.0001, 0.5)
			
func interact():
	if !active:
		print("la la la")
		var tweenx = get_tree().create_tween().set_trans(Tween.TRANS_QUART)
		tweenx.tween_property(Cam.maincam, "fov",$camcontroller/Camera3D.fov, 1.0)
		var tween = get_tree().create_tween()
		tween.tween_property(Cam.maincam, "attributes.dof_blur_amount",$camcontroller/Camera3D.attributes.dof_blur_amount, 1.5)
